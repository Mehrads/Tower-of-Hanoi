...
function add_plate(){
    if(no_of_plates < 9){
        ...
        // Assign a unique color to the new plate
        const colors = ['red', 'blue', 'green', 'yellow', 'purple', 'orange', 'cyan', 'magenta', 'lime'];
        new_plate.style.backgroundColor = colors[no_of_plates - 1];
    }
    else{
...
function start_game(){
    ...
    // Dynamically update the time based on the number of plates
    startTime = no_of_plates * 30; // Example: 30 seconds per plate
    time = startTime;
...
function win_condition(){
    ...
    if((bar1.childNodes.length == 0 && bar2.childNodes.length == 0 && bar3.childNodes.length == no_of_plates) || (bar1.childNodes.length == 0 && bar2.childNodes.length == no_of_plates && bar3.childNodes.length == 0)){
        ...
        // Enable undo and redo after game completion
        undostate.disabled = false;
        redostate.disabled = true; // Initially, there's nothing to redo
